...
class AdjacencyMatrixGraph {
    ...
    showDegreeFromVertex(vertex, log = true) {
        if (!this.isDirected) {
            ...
            if (log)
                console.log(`Vertex ${vertex} has ${degree} degree`);
            return degree;
        }
        ...
        if (log)
            console.log(`Vertex ${vertex} has ${outDegree} degree`);
        return outDegree;
    }
    showAverageDegree() {
    ...
    showMaximumDegree() {
        ...
        console.log(`Maximum degree from graph is ${maximumDegree}. Vertices that has/have this degree is/are ${verticesWithMaximumDegree.join(', ')}`);
        return maximumDegree;
    }
    existsEdgeBetweenVertices(fromVertex, toVertex) {
        console.log(`Between vertices ${fromVertex} and ${toVertex} ${!!this.adjacencyMatrix[fromVertex - 1][toVertex - 1] ? 'exists' : "don't exists"} an edge`);
        return !!this.adjacencyMatrix[fromVertex - 1][toVertex - 1] ? 'exists' : "don't exists";
    }
    showAdjacentVertices(fromVertex) {
    ...
    showMatrix() {
        ...
        this.adjacencyMatrix.forEach((line, i) => {
            console.log(line.map((value, index) => (index === 0 ? `${i + 1}  ` : '') + value + (index + 1 === this.numberOfVertices ? '' : ' | ')).join(''));
        });
        console.log();
    }
}
